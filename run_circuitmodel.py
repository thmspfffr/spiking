          ...
          for iinp in range(0,inputs.size):
              for iampa in range(0,AMPA_mods.size):
                  for inmda in range(0,NMDA_mods.size):
                      ...
                      fn = os.path.expanduser(root_dir + 'proc/spiking_circuitmodel_iampa%d_inmda%d_gaba%d_inp%d_tr%d_v%d_processing.txt') % (iampa, inmda, igaba, iinp,itr,v)
                      ...
                      print("Computing INPUT%d, GABA%d, AMPA%d and NMDA%d, trial%d ...") % (iinp, igaba,iampa,inmda,itr)

                      inp = 1000 * (inputs[iinp]) 
                      inh = 1
                      ...
                      print("Saving output...")
                      hf = h5py.File(os.path.expanduser(root_dir + 'proc/spiking_circuitmodel_iinp%d_ampa%d_nmda%d_gaba%d_tr%d_v%d.h5') % (iinp, iampa, inmda, igaba, itr,v), 'w')
